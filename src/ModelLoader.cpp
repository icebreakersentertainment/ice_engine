...
{
	graphics::model::Mesh data = graphics::model::Mesh();
...
graphics::model::Texture importTexture(const std::string& name, const std::string& filename, uint32 index, const aiMaterial* material, logger::ILogger* logger, fs::IFileSystem* fileSystem)
{
	assert( material != nullptr );
	
	graphics::model::Texture data = graphics::model::Texture();
	...
	if ( material->GetTextureCount(aiTextureType_DIFFUSE) > 0 )
	{
		...
		{
			logger->warn( "graphics::model::Texture not found for model filename: " + filename );
			return data;
		}
		...
		logger->debug( std::string("graphics::model::Texture has filename: ") + fullPath );
		
		if (!fileSystem->exists(fullPath))
		{
			throw std::runtime_error("graphics::model::Texture with filename '" + fullPath + "' does not exist.");
		}
...
graphics::model::Material importMaterial(const std::string& name, const std::string& filename, uint32 index, const aiMaterial* material, logger::ILogger* logger, fs::IFileSystem* fileSystem)
{	
	assert( material != nullptr );
		
	graphics::model::Material data = graphics::model::Material();
...
graphics::model::BoneData importBones(const std::string& name, const std::string& filename, uint32 index, const aiMesh* mesh, logger::ILogger* logger, fs::IFileSystem* fileSystem)
{
	assert( mesh != nullptr );
	
	graphics::model::BoneData boneData = graphics::model::BoneData();
	
	logger->debug( "importing boneData." );
	
	for (uint32 i = 0; i < mesh->mNumBones; i++) {
		graphics::model::Bone data = graphics::model::Bone();
		uint32 boneIndex = 0;
...
graphics::model::BoneNode importBoneNode( const aiNode* node )
{
	graphics::model::BoneNode boneNode = graphics::model::BoneNode();
...
graphics::model::AnimationSet importAnimations(const std::string& name, const std::string& filename, const aiScene* scene, logger::ILogger* logger, fs::IFileSystem* fileSystem)
{
	assert( scene != nullptr );
	
	graphics::model::AnimationSet animationSet = graphics::model::AnimationSet();
	...
	for (uint32 i = 0; i < scene->mNumAnimations; i++)
	{
		...
		graphics::model::Animation animation = graphics::model::Animation();
		...
		for (uint32 j = 0; j < scene->mAnimations[i]->mNumChannels; j++)
		{
			...
			graphics::model::AnimatedBoneNode abn = graphics::model::AnimatedBoneNode();
			...
			// Add graphics::model::AnimatedBoneNode to the animation
		...
		else 
		{
			...
			logger->warn( "graphics::model::Animation with name '" + animation.name + "' already exists!" );
		}
	}
...
std::unique_ptr<graphics::model::Model> importModelData(const std::string& name, const std::string& filename, logger::ILogger* logger, fs::IFileSystem* fileSystem)
{
...
//	Assimp::DefaultLogger::set(new Logger(logger));
	...
	try
	{
		...
		for ( auto& kv : animationSet.animations)
		{
			...
			auto animatedBoneNodes = std::map< std::string, graphics::model::AnimatedBoneNode >();
...
std::unique_ptr<graphics::model::Model> import(const std::string& filename, logger::ILogger* logger, fs::IFileSystem* fileSystem)
{
	...
	std::unique_ptr<graphics::model::Model> model;
	try
	{
