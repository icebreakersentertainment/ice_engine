...
void readNodeHeirarchy(std::vector< glm::mat4 >& transformations, float32 animationTime, const glm::mat4& globalInverseTransform, const std::map< std::string, AnimatedBoneNode >& animatedBoneNodes, const BoneNode& rootBoneNode, const BoneData& boneData, uint32 startFrame, uint32 endFrame, const glm::mat4& parentTransform, std::vector<uint32>& indexCache)
{
	...
	
	{
		...
		if ( it != animatedBoneNodes.end() )
		{
			...
			float32 animationTimeTemp = animationTime;

			// Clamp animation time between start and end frame
			if ( (startFrame > 0 || endFrame > 0) && (startFrame < animatedBoneNode.positionTimes.size() && endFrame < animatedBoneNode.positionTimes.size()))
			{
				float32 st = (float32)animatedBoneNode.positionTimes[startFrame];
				float32 et = (float32)animatedBoneNode.positionTimes[endFrame];

//				std::cout << "B " << animationTimeTemp << " " << startFrame << " " << st << " " << endFrame << " " << et << std::endl;

				auto animationTimeTemp2 = fmod(animationTimeTemp, et - st);
				animationTimeTemp = fmod(animationTimeTemp, et - st) + st;

//				std::cout << "A " << animationTimeTemp2 << " " << animationTimeTemp << " " << startFrame << " " << st << " " << endFrame << " " << et << std::endl;
			...
			glm::vec3 scaling;
			calcInterpolatedScaling(scaling, animationTimeTemp, animatedBoneNode, indexCache);
			glm::mat4 scalingM = glm::scale( glm::mat4(1.0f), glm::vec3(scaling.x, scaling.y, scaling.z) );

			// Interpolate rotation and generate rotation transformation matrix
			glm::quat rotationQ;
			calcInterpolatedRotation(rotationQ, animationTimeTemp, animatedBoneNode, indexCache);
			glm::mat4 rotationM = glm::mat4_cast(rotationQ);

			// Interpolate translation and generate translation transformation matrix
			glm::vec3 translation;
			calcInterpolatedPosition(translation, animationTimeTemp, animatedBoneNode, indexCache);
